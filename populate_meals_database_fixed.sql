-- Complete meal database population script (FIXED VERSION)
-- This script will add all 21 meals with their ingredients and meal items
...
('Water', 0, 0, 0, 0)
) AS new_ingredients(name, kcal, protein, fat, carbs)
WHERE NOT EXISTS (SELECT 1 FROM ingredients WHERE ingredients.name = new_ingredients.name);
...
) AS new_meals(name, category, image, cooking_instructions, is_template, kcal_target)
