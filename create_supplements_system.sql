-- ============================================
-- SUPPLEMENTS & HYDRATION SYSTEM
-- Complete supplement tracking with timing
-- ============================================
...
CREATE TABLE IF NOT EXISTS supplements (
    ...
    name TEXT NOT NULL,
    category TEXT NOT NULL, -- protein, vitamins, minerals, amino_acids, pre_workout, etc.
    description TEXT,
    benefits TEXT[],
    recommended_timing TEXT NOT NULL, -- morning, pre_workout, post_workout, with_meal, before_bed, etc.
    dosage_info TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Create client supplements assignments
CREATE TABLE IF NOT EXISTS client_supplements (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
    supplement_id UUID NOT NULL REFERENCES supplements(id) ON DELETE CASCADE,
    custom_timing TEXT, -- Override default timing if needed
    custom_dosage TEXT, -- Custom dosage instructions
    notes TEXT,
...
CREATE TABLE IF NOT EXISTS client_hydration (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
    target_water_ml INTEGER NOT NULL DEFAULT 3000, -- Daily water goal in ml
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(client_id)
...
CREATE INDEX IF NOT EXISTS idx_supplements_timing ON supplements(recommended_timing);
CREATE INDEX IF NOT EXISTS idx_client_supplements_client ON client_supplements(client_id);
CREATE INDEX IF NOT EXISTS idx_client_supplements_active ON client_supplements(client_id, is_active);
CREATE INDEX IF NOT EXISTS idx_client_hydration_client ON client_hydration(client_id);
...
-- RLS Policies for supplements (read-only for all authenticated users)
CREATE POLICY "Allow all authenticated users to read supplements"
    ON supplements FOR SELECT
    TO authenticated
    USING (true);

-- RLS Policies for client_supplements
CREATE POLICY "Allow authenticated users to read client supplements"
    ON client_supplements FOR SELECT
    TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to insert client supplements"
    ON client_supplements FOR INSERT
    TO authenticated
    WITH CHECK (true);

CREATE POLICY "Allow authenticated users to update client supplements"
    ON client_supplements FOR UPDATE
    TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to delete client supplements"
    ON client_supplements FOR DELETE
    TO authenticated
    USING (true);

-- RLS Policies for client_hydration
CREATE POLICY "Allow authenticated users to read hydration"
    ON client_hydration FOR SELECT
    TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to insert hydration"
    ON client_hydration FOR INSERT
    TO authenticated
    WITH CHECK (true);

CREATE POLICY "Allow authenticated users to update hydration"
    ON client_hydration FOR UPDATE
    TO authenticated
    USING (true);
...
CREATE TRIGGER update_client_supplements_updated_at BEFORE UPDATE ON client_supplements
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_client_hydration_updated_at BEFORE UPDATE ON client_hydration
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ Supplements system created successfully!';
    RAISE NOTICE '📋 Tables: supplements, client_supplements, client_hydration';
    RAISE NOTICE '💊 %s supplements loaded', (SELECT COUNT(*) FROM supplements);
END $$;
