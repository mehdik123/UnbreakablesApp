...
-- Clear existing supplements data (if you want to repopulate)
TRUNCATE TABLE supplements CASCADE;

-- Populate supplements with ON CONFLICT to avoid duplicates
...
DO $$
BEGIN
    RAISE NOTICE '✅ Supplements system populated successfully!';
    RAISE NOTICE '💊 Total supplements: %', (SELECT COUNT(*) FROM supplements);
    RAISE NOTICE '📋 Categories: %', (SELECT COUNT(DISTINCT category) FROM supplements);
END $$;
